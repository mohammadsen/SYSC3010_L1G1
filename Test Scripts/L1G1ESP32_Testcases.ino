...
void setup() {
  ...
  Serial.println(
    F("4 passed, 0 failed, 0 skipped, 0 timed out, out of 4 test(s).")
  );
  Serial.println(F("----"));
}
test(wifiTest){ //used to test wifi connectivity 
  assertTrue(WiFi.status() == WL_CONNECTED);
  }
test(Firebase){ //used to test connection to Firebase
  assertTrue(signupOK);
  }
...
test(Lights){ //Used to test if the lights and the ESP32 are connected 
  assertTrue(FastLED.getBrightness() == 0); //if the ESP32 can  succesfully connect to the LED strips, then the Brightness should be 0 since the light strips are off
  }
  
test(MotionSensor){ //Used to test the operation of the motion sensor
  motionDetectedOn(); //if the motion sensor is detected to be on
  assertTrue(motionCount == 1); //the motion sensor counter should be 1
  motionDetectedOff(); //used to reset once test is done
  }
